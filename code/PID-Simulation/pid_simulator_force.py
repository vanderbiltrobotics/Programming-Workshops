...
def run_simulation(target_array, time_array, ext_force_arr,  p_gain, i_gain, d_gain):
    ...
    for step in range(len(time_array)):
        ...
        applied_force = (p_gain * pos_error)
        applied_force += (i_gain * integrator_sum)
        applied_force += (d_gain * error_slope)
...
run_simulation(target_const, time_arr, ext_force_const, P_gain, I_gain, D_gain)
